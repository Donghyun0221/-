...
# length(grp3)
grp54=c(rep(1,25), rep(2,9), rep(3,26), rep(4,19), rep(5,15),
        ...
        rep(21,1), rep(22,1), 25, rep(23,2), rep(24,1), rep(25,8),
        rep(26,1), rep(27,1), rep(28,1), rep(29,1), rep(30,1),
        rep(31,1), rep(32,1), rep(33,11), rep(34,1), rep(35,6), rep(36,1), rep(37,3), rep(38,11), rep(39,1), rep(40,3), rep(41,3), 42, 43, 42, 44, 45, 44, rep(46,2), rep(47,3), 48, 49, 50, rep(51,4), 52, rep(51,2), 53, 54)
length(grp54)
# 산업분류_165_고용자수
# grp3_labor=c(rep(1,8),rep(2,109),rep(3,48))
# length(grp3_labor)
grp54_labor=c(rep(1,8), rep(2,4), rep(3,11), rep(4,6), rep(5,6),
              ...
              rep(21,1), rep(22,1), 25,  rep(23,1), rep(24,1), rep(25,6), 26, 27, 28, 29, 30, 31, 32,
              rep(33,7), rep(34,1), rep(35,3), rep(36,1), rep(37,2), rep(38,5), rep(39,1), rep(40,3), rep(41,2), 42, 43, 42, rep(44,1), 45, 44, 46, 47, 48, 49, 50, rep(51,2), 52, rep(51,2), 53, 54)
length(grp54_labor)


# file.choose()
inter = read.xlsx("C:\\Users\\donghyun\\Desktop\\김동현\\산업연관분석 프로젝트\\투입산출표_생산자가격_기본부문 by Song.xlsx")
labor = read.xlsx("C:\\Users\\donghyun\\Desktop\\김동현\\산업연관분석 프로젝트\\외국인 고용표.xlsx")
...
# setting
n_sec_inter=381; n_sec_labor=179
n_vadded=6
...
# merge_mat: 행열 동시 통합
inner_merge=merge_mat(inner,grp54)
sec_names_inter_merge = sec_names <- c("농림수산품","광산품","음식료품","섬유_가죽제품","목재_종이_인쇄",
                                       ...
                                       "건설","도소매_및_상품중개서비스", "철도서비스","도로운송서비스", "수상운송서비스","항공운송서비스","기타화물운송및운송보조서비스","일반음식점서비스","기타음식점서비스", "주점", 
                                       "비알콜음식점","숙박서비스","정보통신및방송서비스","무선및위성통신서비스","정보통신및방송서비스","은행서비스","금융및보험서비스","주거임대서비스","부동산서비스","전문과학및기술서비스",
                                       "장비및용품대여","사업지원서비스","공공행정및사회보장서비스","교육서비스","비영리교육서비스","의료서비스","비영리의료서비스","사회복지서비스",
                                       "문화및예술서비스","여행사서비스","스포츠서비스","오락서비스","기타서비스","미용서비스","기타개인서비스","기타")
length(sec_names)
...
# merge_col: 열만통합_노동자수
labor_merge <- merge_col(labor_num, grp54_labor)
...
# merge_col: 열만통합_부가가치
value_merge <- merge_col(value, grp54)
...
#print(impact)
impact=c(rep(0,19),4736809,158949,69306,16403,1455828,0,1348401,161471,1017618,141908,3692718,0,191140,0,471,0,5984,0,0,197177,0,0,10994,0,212516,0,0,17179,159710,99843,614929,81294,18669,1143,0); length(impact)
length(impact)
